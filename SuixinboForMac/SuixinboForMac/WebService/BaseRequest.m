...
- (void)parseResponse:(NSObject *)respJsonObject
{
            ...
            if (_succHandler)
            {
                ...
                else
                {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        NSString *msg = [NSString stringWithFormat:@"mac 返回数据格式有误--> %@",respJsonObject];
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    {
                        NSString *msg = [NSString stringWithFormat:@"mac返回的数据有业务错误--> %@",[_response message]];
                        [[ILiveSDK getInstance] iLiveLog:@"DEV" level:ILive_LOG_DEBUG pos:@"Baserequest->parseResponse" msg:msg ext:nil];
                        // 返回的数据有业务错误
                        if (_failHandler)
                        {
    ...
    {
        NSString *msg = [NSString stringWithFormat:@"mac返回数据为空--> %@",[self class]];
        [[ILiveSDK getInstance] iLiveLog:@"DEV" level:ILive_LOG_DEBUG pos:@"Baserequest->parseResponse" msg:msg ext:nil];
